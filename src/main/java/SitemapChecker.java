...
public class SitemapChecker {
    ...
    private static final boolean CHECK_BLOGS = true;           // /sitemaps/blogs/
    ...
    // ⚙️ DİL FİLTRLƏRİ (boş list = hamısı, dolu list = yalnız bunlar)
    // Məsələn: yalnız EN və RU yoxlamaq üçün:  {"en", "ru"}
    private static final Set<String> LANGUAGE_FILTER = new HashSet<>(Arrays.asList(
            "en" // Yalnız bunları yoxla (comment out = hamısını yoxla)
    ));
